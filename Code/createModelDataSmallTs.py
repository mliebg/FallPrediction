...
def main():
    ...
    src_dir_path = config['data']['input_dir']        # directory of sensor data (CSVs)
    out_dir_path = config['data']['output_dir']       # output directory of model data (pickle)
    time_steps = config['data']['time_steps']         # number of timesteps that are used in input range
    trigger_range = config['data']['trigger_range']   # time before a fall that classifies as willFall later in model (in seconds)
