...
def main():
    ...
    print_stamped_message(2, 'TEST RUN PROCESS A LOG FILE')
    ...
    logfile_path = config["eval"]["logfile_path"]   #'/home/dev/repos/FallPrediction/DataAcquisition/DataLabeling/FullyLabeled/data/excludeForTests/imu_5pathfinder-11-25-34_labeled.csv'
    model_path = config["eval"]["model_path"]       # path to keras model
    timesteps = config["data"]["time_steps"]
    trigger_range = config['data']['trigger_range']

    if script_chaining:
        print_stamped_message(2,f'BEGIN TESTRUN ON LOG DATA\n Parameter Settings:\n  logfile dir:   {logfile_path}\n  model path:    {model_path}')
    else:
        print_stamped_message(2,f'BEGIN TESTRUN ON LOG DATA\n Parameter Settings:\n  logfile dir:   {logfile_path}\n  model path:    {model_path}\n  time steps:    {timesteps}\n  trigger range: {trigger_range}\n')
    ...
    for index, row in log_ds.iterrows():
        ...
        if index >= timesteps -1:
            ...
            X_np = np.array(ql)
            #print(X_np.shape)
            X_resh = np.reshape(X_np, (1, timesteps, 6))

            truth = trigger_range * 10 ** 6 >= row['willFall_dt']
    ...
    result_filename = f'resultLstm35-{dt.datetime.now().strftime("%Y%m%dT%H%M%S")}.json'
    result_data = {
        'timesteps'     : timesteps,
